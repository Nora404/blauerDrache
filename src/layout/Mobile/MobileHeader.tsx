...
const MobileHeader: React.FC<MobileHeaderProps> = observer(() => {
    ...
    const percentage = playerBase.store.nextLevel > 0 ? (playerBase.store.exp / playerBase.store.nextLevel) * 100 : 0;
    ...
    return (
        <div className='flex-row w-full'>
            ...
            <div id="header0" className='text-left'>
                {playerMeta.store.name}<br />
                Titel: {playerMeta.store.titel}
            </div>
            ...
            <div id="header2" className='text-left'>
                {SYSTEM.Level}: {playerBase.store.level}<br />
                {SYSTEM.Erfahrung}: {formattedPercentage}%
            </div>
            <div id="header3" className='text-left'>
                {SYSTEM.Gold}: {playerEconomy.store.gold || '0'}<br />
                {SYSTEM.Edelsteine}: {playerEconomy.store.edelsteine || '0'}
            </div>
