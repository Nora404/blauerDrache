...
const CreatePlayer: React.FC = observer(() => {
	const { playerBase, playerStats, playerEconomy, playerMeta, gameState, resetGameData } =
		useRootStore();
	...
	return (
		<div className="max-width">
			...
			{currentStep === 0 && (
				<ChooseRace wizardData={wizardData} setWizardData={setWizardData} onNext={handleGoNext} />
			)}
			{currentStep === 1 && (
				<ChooseOrigin
			...
			{currentStep === 4 && (
				<>
					<button className="btn-border" onClick={handleLastBtn}>
						Reise beginnen
					</button>
				</>
			)}
