...
import { emptyWeaponObj, weapons } from "../../../../data/gameItems/weaponData";
...
const WeaponBuy: React.FC<WeaponBuyProps> = observer(() => {
  ...
  const refund = currentWeapon.ek
    ? Number((currentWeapon.ek / 1.2).toFixed())
    : 0;
  //#endregion
  ...
  return (
    <div className="max-width">
      ...
      <p className="mb-1 text-left">
        Du zeigst interesse an <b>{localWeapon.name}</b> und liest an einem
        kleinen Schild, dass es <Talk color="rot">{localWeapon.ek}</Talk>{" "}
        {SYSTEM.Gold} kostet. Nach einem Blick in deinen Geldbeutel erkennst du
        dass, {showText()}. Der Verkäufer schaut dich erwartungsvoll an. Seine
        Blicke wandern zu deiner Hand, in der sich <b>{currentWeapon.name}</b>{" "}
        befindet. Er sagt:{" "}
        <GradientText colors={["#D1813B"]}>
          "Ich nehme es dir gerne ab, für
        </GradientText>{" "}
        <Talk color="grün">{refund}</Talk> {SYSTEM.Gold}."
      </p>
        ...
        {weapons.map((weapon: Item) =>
          weapon.name !== "Nichts" ? (
            <ItemCard
              ...
              key={weapon.name}
              onClick={handleClick}
              isActive={localWeapon.name === weapon.name}
            />
          ) : null
        )}
      </div>
