	...
	return (
		<div className="max-widht">
			...
			{buttons.map((btn, index) => (
				<div
					key={index}
					style={{ border: "1px dashed gray", padding: "15px", marginBottom: "25px" }}>
					<details open={true}>
						<summary>
							<b>
								<MultiColoredLetters colors={yellowColors}>Neuer Button:</MultiColoredLetters>
						...
						<div className="form-group" style={{ marginTop: "0.5rem" }}>
							<TextEditor
								...
								onChange={(newVal) => {
									setButtons((prev) =>
										prev.map((b, i) => (i === index ? { ...b, message: newVal } : b))
									);
								}}
							/>
						</div>
							...
							<label>
								<input
									...
									onChange={(e) => {
										...
										setButtons((prev) =>
											prev.map((b, i) => (i === index ? { ...b, conditionsEnabled: checked } : b))
										);
									}}
								/>
						...
						{btn.conditionsEnabled && <ConditionsEditor button={btn} index={index} />}

						{/* Ganzen Button entfernen */}
						<button onClick={() => removeButton(index)} className="remove-button w-100">
