	...
	const loadEvent = (ev: GameEvent) => {
		...
		const buttonConfigs: ButtonConfig[] = ev.buttons.map((btn) => {
			...
			return {
				...
				fluxDelta: {
					feeling: typeof action.fluxDelta?.feeling === "string" ? action.fluxDelta.feeling : "",
					buff: typeof action.fluxDelta?.buff === "string" ? action.fluxDelta.buff : "",
					debuff: typeof action.fluxDelta?.debuff === "string" ? action.fluxDelta.debuff : "",
					item: typeof action.fluxDelta?.item === "string" ? action.fluxDelta.item : "",
				},
