...
import ColoredText from "../Formatted/ColoredText";
//#endregion
...
function parseCustomComponents(text: string): Array<string | React.ReactNode> {
	const pattern = /{(GradientText|MultiColoredLetters|Talk|ColoredText)\|([^}]+)}([\s\S]*?){\/\1}/g;
	...
	while ((match = pattern.exec(text)) !== null) {
		...
		} else if (compName === "ColoredText") {
			const [col, form] = paletteOrColor.split("|");
			element = (
				<ColoredText key={matchIndex} color={col} format={form as any}>
					{innerText}
				</ColoredText>
			);
		}
...
//#region
function insertLineBreaks(nodes: Array<string | React.ReactNode>): React.ReactNode {
